...
class User extends MY_Controller {
	...
	public function list_User(){
		...
		if(!empty($cek) && $stts=='operator')
        {
			...
			$inside['content']=$this->load->view('operator/User/list_User',$data, TRUE);
	...
	public function hapus_User(){
		...
		if(!empty($cek) && $stts=='operator')
		{
			...
			header('location:'.base_url().'operator/User/list_User');
		}
		else
		{
			 header('location:'.base_url().'global/login/log');
	...
	public function view_User(){
		...
		if(!empty($cek) && $stts=='operator')
        {
			...
			$inside['content']=$this->load->view('operator/User/view_User',$data, TRUE);
			$this->load->view('operator/layout',$inside);
		}
		else
        {
	...
	public function add_User(){
		...
		if(!empty($cek) && $stts=='operator')
        {
				...
				if($this->form_validation->run()==FALSE)
				{
					...
					$inside['content']=$this->load->view('operator/User/add_User',$data, TRUE);
					$this->load->view('operator/layout',$inside);
				}
			 	else
				{
					...
					if (empty($gambar))
					{
								...
								if ($isi == 0) {
									...
									redirect("operator/User/list_User","refresh");
								}
								else
								{
	...
	public function edit_User(){
		...
		if(!empty($cek) && $stts=='operator')
        {
			...
			$id = $this->input->get('username', TRUE);	
			$data['user'] = $this->Simple_CRUD->get_detail1('tb_user','username',$id);
			$inside['content']=$this->load->view('operator/User/edit_User',$data, TRUE);
			$this->load->view('operator/layout',$inside);
		}
		else
        {
	...
	public function update_User(){
 	...
 	public function edit_password(){
		...
		if(!empty($cek) && $stts=='operator')
        {
			...
			$inside['content']=$this->load->view('operator/User/edit_password',$data, TRUE);
			$this->load->view('operator/layout',$inside);
		}
		else
        {
